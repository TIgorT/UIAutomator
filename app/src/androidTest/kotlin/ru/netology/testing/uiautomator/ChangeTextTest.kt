...
class ChangeTextTest {
    ...
    private val textToSetSecond = ""
    private val textToSetInitial = "Hello UiAutomator!"
    private  val  newText = "But still funny"
    ...
    @Test
    fun textViewRemainedTheSame(){

        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textToSetSecond
        device.findObject(By.res(packageName, "buttonChange")).click()

        val result = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(result, textToSetInitial)
    }

    @Test
    fun testWithTheAppearanceOfActivityOnTheScreen(){
